...
class NeuralNetwork:
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        if self.num_layers == 2:
            grads['W2'] = (np.dot(self.nonlinear(layer_output[1].T),grad_scores))       
            grads['b2'] = np.sum(grad_scores, axis = 0, keepdims = True)               

            hidden_layer1 = (grad_scores.dot(self.params['W2'].T))                     
        ...
        if self.num_layers == 3: 
            grads['W3'] = np.dot(self.nonlinear(layer_output[2].T),grad_scores)       
