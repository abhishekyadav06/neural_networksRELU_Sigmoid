...
class NeuralNetwork(object):
    ...
    def __init__(self, input_size, hidden_sizes, output_size, num_layers, nonlinearity='relu'):
        ...
        std = 1e-4
        self.params = {}
        self.params['W1'] = std * np.random.randn(input_size, hidden_sizes[0])
        self.params['b1'] = np.zeros(hidden_sizes[0])
        self.params['W2'] = std * np.random.randn(hidden_sizes[0], output_size)
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        relu_1 = self.nonlinear(layer_1)#np.maximum(0, layer_1)
        ...
        y_correct = scores_exp[range(N), list(y)]  # [N, 1]
        loss = np.sum(-np.log(y_correct / np.sum(scores_exp, axis=1)))
        ...
        grad_layer_1 = self.nonlinear_grad(relu_1)*grad_relu_1 #(relu_1 > 0) * grad_relu_1
